...
resource "aws_instance" "frontend-ec2" {
  ...
  "sudo systemctl reload nginx"
   ...
   connection {
    type        = "ssh"
    user        = var.ssh_user
    private_key = file("./fundoo-frontend-key.pem")
    host        = self.public_ip
    port        = var.ssh_port
...
resource "null_resource" "database_setup" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      ...
      private_key         = file("./fundoo-database-key.pem")
      ...
      bastion_private_key = file("./fundoo-frontend-key.pem")
    }
  }
...
resource "null_resource" "backend_setup" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type                = "ssh"
      user                = var.ssh_user
      private_key         = file("./fundoo-backend-key.pem")
